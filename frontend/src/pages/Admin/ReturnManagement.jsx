...
const ReturnManagement = () => {
  ...
  const {
    ...
    isLoading,
    refetch,
  } = useGetReturnRequestsQuery(activeTab === "all" ? "" : activeTab);

  const [reviewReturn, { isLoading: isReviewing }] =
    useReviewReturnRequestMutation();
  const [markPickedUp, { isLoading: isPickingUp }] =
    useMarkReturnPickedUpMutation();
  ...
  const handleAction = async (action, id) => {
    try {
      if (action === "approve") {
        ...
        await reviewReturn({
          id,
          body: { action: "reject", adminNotes: rejectNotes },
        }).unwrap();
  ...
  const openModal = (ret, type) => {
    setSelectedReturn(ret);
    setModalType(type);
    if (type === "refund") {
      const maxRefund = ret.items.reduce(
      ...
      <div className="max-w-7xl mx-auto px-6 py-8">
        ...
        {isLoading ? (
          <div className="space-y-4">
            <div className="h-24 bg-gray-200 animate-pulse rounded-sm"></div>
          </div>
                  ...
                  <div className="lg:col-span-3">
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-4">
                      <div>
                        ...
                        <p className="text-sm text-gray-500">
                          by {ret.user?.username || "Unknown"} (
                          {ret.user?.email})
                        </p>
                      </div>
                      <span
                        className={`mt-2 sm:mt-0 px-3 py-1 text-sm font-semibold uppercase rounded-sm w-fit
                        ${
                    ...
                    <div className="flex gap-4 mb-4 overflow-x-auto pb-2">
                      {ret.items.map((item, i) => (
                        <div
                          key={i}
                          className="flex items-center gap-3 bg-gray-50 p-2 border border-gray-100 min-w-[200px]"
                        >
                          {item.image ? (
                            <img
                              src={item.image}
                          ...
                          <div className="text-sm">
                            <p className="font-medium text-black text-xs truncate max-w-[120px]">
                              {item.name}
                            </p>
                    ...
                    {/* ✅ নতুন যোগ করা হয়েছে: Description */}
                    {ret.returnDescription && (
                      <p className="text-sm text-gray-600 italic mb-3 bg-gray-50 p-2 rounded border border-gray-100 inline-block mt-2">
                        {ret.returnDescription}
                      </p>
                    )}

                    {/* ✅ নতুন যোগ করা হয়েছে: Image Gallery */}
                  ...
                  <div className="flex flex-row lg:flex-col gap-2 items-start lg:items-end justify-end">
                    {ret.returnStatus === "requested" && (
                      <>
                        <button
                          ...
                          className="flex-1 lg:w-full flex items-center justify-center gap-2 px-4 py-2 border border-green-600 text-green-600 hover:bg-green-600 hover:text-white transition-colors text-sm font-medium disabled:opacity-50"
                        >
                          <FaCheck size={12} /> Approve
                        </button>
                        <button
                          onClick={() => openModal(ret, "reject")}
                          className="flex-1 lg:w-full flex items-center justify-center gap-2 px-4 py-2 border border-red-600 text-red-600 hover:bg-red-600 hover:text-white transition-colors text-sm font-medium"
                        >
                    ...
                    {ret.returnStatus === "approved" && (
                      <button
                        ...
                        className="flex-1 lg:w-full flex items-center justify-center gap-2 px-4 py-2 border border-indigo-600 text-indigo-600 hover:bg-indigo-600 hover:text-white transition-colors text-sm font-medium disabled:opacity-50"
                      >
                    ...
                    {(ret.returnStatus === "approved" ||
                      ...
                      <button
                        ...
                        className="flex-1 lg:w-full flex items-center justify-center gap-2 px-4 py-2 bg-black text-white hover:bg-red-600 transition-colors text-sm font-medium"
                      >
        ...
        {selectedReturn && modalType && (
          ...
          >
            <motion.div
              ...
              className="bg-white w-full max-w-md p-6 border border-gray-200"
              onClick={(e) => e.stopPropagation()}
            >
              {/* ✅ নতুন যোগ করা হয়েছে: Image Preview Modal */}
              {modalType === "image" && (
                <div>
                  ...
                  <img
                    src={previewImage}
                    alt="Proof"
                    className="w-full h-auto max-h-[70vh] object-contain border border-gray-100 rounded"
                  />
                </div>
              )}
              ...
              {modalType === "reject" && (
                <div>
                  ...
                  <textarea
                    ...
                    className="w-full border border-gray-300 p-3 text-sm focus:border-black outline-none resize-none mb-4"
                    placeholder="Tell the customer why..."
                  />
                  <div className="flex gap-3">
                    <button
                      onClick={closeModal}
                      className="flex-1 py-3 border border-gray-300 text-gray-700 font-medium hover:border-black transition-colors"
                    >
                      Cancel
                    </button>
                    <button
                      ...
                      disabled={isReviewing}
                      className="flex-1 py-3 bg-red-600 text-white font-medium hover:bg-red-700 transition-colors disabled:opacity-50"
                    >
                      Confirm Reject
                    </button>
                  </div>
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex gap-3 pt-2">
                    ...
                    <button
                      ...
                      className="flex-1 py-3 bg-black text-white font-medium hover:bg-green-600 transition-colors disabled:opacity-50"
                    >
                      Confirm Refund
                    </button>
                  </div>
