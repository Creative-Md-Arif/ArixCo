...
const StatsSkeleton = () => (
  <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
    {[...Array(4)].map((_, i) => <div key={i} className="bg-white border border-gray-200 p-4 rounded-sm h-24 animate-pulse"></div>)}
  </div>
);
...
const BannerCard = memo(function BannerCard({ 
  banner, index, handleToggleStatus, handleDelete, getTypeLabel, togglingId, deletingId 
}) {
  return (
    <motion.article
    ...
    >
      ...
      <div className="relative h-40 overflow-hidden bg-gray-50 border-b border-gray-200">
        ...
        <div className="absolute top-2 right-2">
          <span className={`border text-sm font-bold uppercase tracking-widest px-2 py-1 rounded-sm ${banner.isActive ? "bg-white text-black border-black" : "bg-white text-gray-500 border-gray-300"}`}>
      ...
      <div className="p-4">
        ...
        <div className="flex gap-4 mb-4 text-sm text-gray-500 font-bold uppercase tracking-wider border-t border-b border-gray-100 py-2">
          <span className="flex items-center gap-1">
            <FaMousePointer size={12} className="text-gray-400" /> {banner.clicks || 0}
          </span>
          <span className="flex items-center gap-1">
            <FaEye size={12} className="text-gray-400" /> {banner.impressions || 0}
          </span>
        </div>
        ...
        <div className="flex gap-2">
          ...
          >
            {togglingId === banner._id ? (
              ...
              <>
                {banner.isActive ? <FaEyeSlash size={12} /> : <FaEye size={12} />}
...
const BannerList = () => {
  ...
  const bannerTypes = useMemo(() => [
    { value: "hero", label: "Hero" }, { value: "category", label: "Category" },
    { value: "promotional", label: "Promo" }, { value: "sidebar", label: "Sidebar" },
  ...
  const getTypeLabel = (type) => bannerTypes.find((t) => t.value === type)?.label || type;

  // Reusable Select Style
  const selectClass = "bg-white border border-gray-200 rounded-sm px-4 py-2.5 text-sm font-bold uppercase tracking-wider focus:ring-1 focus:ring-black focus:border-black outline-none transition-all cursor-pointer font-['Trebuchet_MS']";
        ...
        <div className="max-w-[1500px] mx-auto">
          ...
          <header className="mb-8 flex flex-col md:flex-row md:items-end justify-between border-l-4 border-black pl-6 py-2 gap-4">
            ...
            <Link to="/admin/banner/create">
              <button className="bg-black text-white px-5 py-3 text-sm font-bold uppercase tracking-widest hover:bg-red-600 transition-all flex items-center gap-2 rounded-sm w-full md:w-auto justify-center">
                <FaPlus size={12} /> Create Banner
              </button>
            </Link>
          </header>
          ...
          {statsLoading ? <StatsSkeleton /> : stats && (
            <section className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
              <div className="bg-white border border-gray-200 p-4 rounded-sm hover:border-black transition-colors flex items-center gap-3">
                ...
                <div>
                  <p className="text-sm uppercase tracking-widest text-gray-500 font-bold">Total Banners</p>
                  <p className="text-xl font-black text-black font-['Playfair_Display']">{stats.totalBanners}</p>
                </div>
              </div>
              <div className="bg-white border border-gray-200 p-4 rounded-sm hover:border-black transition-colors flex items-center gap-3">
                <FaEye className="text-xl text-gray-400" />
                <div>
                  <p className="text-sm uppercase tracking-widest text-gray-500 font-bold">Active</p>
                  <p className="text-xl font-black text-black font-['Playfair_Display']">{stats.activeBanners}</p>
                </div>
              </div>
              <div className="bg-white border border-gray-200 p-4 rounded-sm hover:border-black transition-colors flex items-center gap-3">
                <FaMousePointer className="text-xl text-gray-400" />
                <div>
                  <p className="text-sm uppercase tracking-widest text-gray-500 font-bold">Total Clicks</p>
                  <p className="text-xl font-black text-black font-['Playfair_Display']">{stats.byType?.reduce((acc, t) => acc + t.totalClicks, 0) || 0}</p>
                </div>
              </div>
              <div className="bg-white border border-gray-200 p-4 rounded-sm hover:border-black transition-colors flex items-center gap-3">
                <FaChartBar className="text-xl text-gray-400" />
                <div>
                  <p className="text-sm uppercase tracking-widest text-gray-500 font-bold">Impressions</p>
                  <p className="text-xl font-black text-black font-['Playfair_Display']">{stats.byType?.reduce((acc, t) => acc + t.totalImpressions, 0) || 0}</p>
                </div>
              </div>
            </section>
          )}
          ...
          <section className="flex flex-col sm:flex-row gap-3 mb-6 p-4 border border-gray-200 rounded-sm bg-white">
            <select value={filterType} onChange={(e) => setFilterType(e.target.value)} className={`${selectClass} w-full sm:w-auto`}>
          ...
          {!isLoading && filteredBanners?.length === 0 && (
            <div className="text-center py-20 border border-dashed border-gray-200 rounded-sm bg-white">
              ...
              <p className="text-gray-500 font-bold uppercase tracking-widest text-sm">No banners found</p>
              <Link to="/admin/banner/create" className="inline-block mt-4 text-red-600 font-bold uppercase tracking-widest text-sm hover:underline">
